  ...
  describe '.validate' do
    context 'when validation is successful' do
      context 'returns a validation object with no errors' do
        let(:valid1) { TinFormat.validate('10120000004', 'au') }
        let(:valid2) { TinFormat.validate('101200000', 'au') }
        let(:valid3) { TinFormat.validate('123456789', 'ca') }
        let(:valid4) { TinFormat.validate('22BCDEF1G2FH1Z5', 'in') }
    ...
    context 'when validation fails' do
      context 'returns a validation object with errors' do
        let(:valid1) { TinFormat.validate('101200B0004', 'au') }
        let(:valid2) { TinFormat.validate('1234567890000', 'ca') }
        let(:valid3) { TinFormat.validate('22BCDEF1G', 'in') }
